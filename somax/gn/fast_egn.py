"""
Scaling Gauss-Newton Method for Multi-Class Classification
"""
...
class FastEGNState(NamedTuple):
    ...
    # debug info
    grad_inf_norm: Optional[float]
    jac_inf_norm: Optional[float]
    q_inf_norm: Optional[float]
    direction_inf_norm: Optional[float]



@dataclasses.dataclass(eq=False)
class FastEGN(base.StochasticSolver):
    # Jacobian of the residual function
    predict_fun: Callable
    jac_fun: Optional[Callable] = None
    loss_fun: Optional[Callable] = None
    grad_fun: Optional[Callable] = None  # needed only for multi-class classification
    ...
    def __post_init__(self):
        ...
        # Classification (Cross-Entropy)
            ...
            if self.n_classes == 1:  # binary classification
                ...
                # TODO: special tricks for multi-class classification
                if self.loss_fun is None:
                    self.loss_fun = self.ce

                self.grad_fun = jax.grad(self.loss_fun)

                self.jac_fun = jax.vmap(jax.grad(self.predict_true_class, has_aux=True), in_axes=(None, 0, 0,))

                self.calculate_direction = self.calculate_direction_ce

                self.regularizer_array = self.regularizer * jnp.eye(self.batch_size)
        else:
    ...
    def update(
        ...
        direction, grad_loss, J, debug_vals = self.calculate_direction(params, state, targets, *args)
        ...
        next_state = FastEGNState(
            ...
            grad_inf_norm=debug_vals[0],
            jac_inf_norm=debug_vals[1],
            q_inf_norm=debug_vals[2],
            direction_inf_norm=debug_vals[3],
        )
    ...
    def calculate_direction_ce(self, params, state, targets, *args):
        ...
        jac_tree, batch_logits = self.jac_fun(params, targets, *args)

        # 2nd most time-consuming part - calculate the gradient of the loss function
        grad_tree = self.grad_fun(params, *args, targets)

        # flatten pytrees
        J = flatten_2d_jacobian(jac_tree)
        grad_loss = ravel_pytree(grad_tree)[0]
        ...
        norm_jac = jnp.linalg.norm(J, axis=None)
        min_q = jnp.min(1 - batch_probs_of_true_class)
        norm_dir = jnp.linalg.norm(direction)
        debug_vals = (norm_grad, norm_jac, min_q, norm_dir)

        return direction, grad_loss, J, debug_vals
    ...
    def predict_true_class(self, params, targets, *args):
        preds = self.predict_fun(params, *args)
