...
def create_solver(
    ...
    elif solver_type.startswith('ng'):
        # switch case for loss functions
        if solver_type == 'ng':
            loss_fn_candidate = ce
        elif solver_type == 'ng-poly1':
            loss_fn_candidate = ce_poly1
        elif solver_type == 'ng-poly-a':
            loss_fn_candidate = ce_poly
        elif solver_type == 'ng-logeps':
            loss_fn_candidate = ce_logeps
        elif solver_type == 'ng-logbarrier':
            loss_fn_candidate = ce_logbarrier
        else:
            raise ValueError(f'Unknown NG solver: {solver_type}')
...
if __name__ == '__main__':
    ...
    @jax.jit
    def ce_poly1(params, features, targets):
        # b x C
        logits = predict_fn(params, features)

        residuals = poly_loss_cross_entropy(logits=logits, labels=targets)
        ...
        for solver_type, solver_params in task_hps.items():
            for config in configs[solver_type]:
                for seed in range(N_SEEDS):
                    ...
                    # --------------- init solvers ---------------
                    is_egn_like = solver_type in ['egn', 'hfo', 'sgn', ] or solver_type.startswith('ng')
                    b = config[0]
